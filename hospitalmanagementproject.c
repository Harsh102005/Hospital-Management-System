...
//     float price;
//     int qty;
//     time_t timestamp;  // Timestamp for when the item is purchased
// };
...
//     char disease[50];
//     char city[30];
//     char mobile[10];  // Changed to char array to handle phone numbers
//     char doctor[50];
// };

// struct order {
//     struct patient pat;  // Patient details
//     char date[50];
//     int numofItems;
//     struct items itm[50];
// };
...
//     printf("\nCity: %s", pat.city);
//     printf("\nMobile: %s", pat.mobile);  // Modified to print mobile as string
//     printf("\nDoctor: %s", pat.doctor);
//     printf("\n----------------------------------------------------------------------------");
...
//     strftime(time_str, 50, "        %m-%Y-%d %H:%M:%S", tm_info); // Format the timestamp

//     printf("\n%s\t\t%d\t\t%.2f\t\t%s", item, qty, qty * price, time_str); // Print timestamp
// }
...
//     struct order ord;
//     FILE *fp;
//     char input_name[50];  // Used to store input for search
...
//         printf("\n\n Your Choice: ");
//         scanf("%d", &opt);
//         fgetc(stdin); // Clear newline character from input buffer
...
//                 fgets(ord.pat.mobile, 15, stdin);  // Now using fgets for mobile number
//                 ord.pat.mobile[strlen(ord.pat.mobile) - 1] = '\0';  // Remove newline character
...
//                 printf("Enter the number of treatment items: ");
//                 scanf("%d", &n);
//                 ord.numofItems = n;
...
//                     ord.itm[i].timestamp = time(NULL); // Capture the current time when the item is added
...
//                 input_name[strlen(input_name) - 1] = '\0';  // Remove newline character
...
//                 while (fread(&ord, sizeof(struct order), 1, fp)) {
//                     float tot = 0;
//                     if (strcmp(ord.pat.name, input_name) == 0) {  // Compare with input name
//                         BillBana(ord.pat, ord.date);
//                         for (int i = 0; i < ord.numofItems; i++) {
//                             Kaisa_Banau_Bill(ord.itm[i].item, ord.itm[i].qty, ord.itm[i].price, ord.itm[i].timestamp);
//                             tot += ord.itm[i].qty * ord.itm[i].price;
